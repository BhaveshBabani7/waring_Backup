...
function init(args, preRiskCall) {
    ...
    var paymentTokenMap = {
        ...
        PayPal: KHash.hashGiftCard(order.paymentInstrument.custom.paypalPayerID)
    };
	...
	var paypalFixEnable = Site.getCurrent().getCustomPreferenceValue('paypalFixEnable');
    ...
    if (constants.RISK_WORKFLOW_TYPE === constants.RISK_WORKFLOW_TYPE_PRE && !preRiskCall && orderID && args.Order.custom.kount_Status.value !== 'RETRY') {
		...
		if (paypalFixEnable && paymentType === 'PYPL' && !empty(paymentToken)) { //eslint-disable-line
            RequiredInquiryKeysVal.PENC = 'KHASH';
			RequiredInquiryKeysVal.PTYP = paymentType;
        }
    } else {
        if (paymentType === 'PYPL' && paypalFixEnable) {
            paymentToken = '';
        }
        RequiredInquiryKeysVal = {
    ...
    try {
        ...
        if (!empty(response)) {
            if (constants.RISK_WORKFLOW_TYPE === constants.RISK_WORKFLOW_TYPE_PRE && !preRiskCall && orderID && args.Order.custom.kount_Status.value !== 'RETRY') {
                ...
                // Write initial Kount status to the order note
                var storeStatus = Site.getCurrent().getCustomPreferenceValue('storeInitialKountStatus');
                if (storeStatus) {
                    kount.addOrderNote(args.Order, 'Initial Kount status', responseArgs.KountOrderStatus);
                }
            }
