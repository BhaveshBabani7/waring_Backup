...
function call_acoustic(cart){
    if (app.getForm('cart').object.VarAcous.value == 'true') {
...
function initAddressForm(cart) {
    ...
    try {
        Transaction.wrap(function () {
           basket.custom.OrderPurpose = app.getForm('cart').object.forPurpose.value;
        });
   } catch (error) {
       Transaction.wrap(function () {
...
function start(cart, params) {
    ...
    //avatax multiple calling issue solution
     var arg=false;
     if(session.forms.singleshipping.shippingAddress.addressFields.postal.value != null ) {
         arg=true;
     }
    Transaction.wrap(function () {
        cart.calculate(arg);
